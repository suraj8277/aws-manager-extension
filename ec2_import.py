...
def _get_ssm_statuses(session: boto3.Session, region: str, instance_ids: List[str]) -> Dict[str, str]:
    ...
    try:
        ...
        # describe_instance_information can handle a max of 50 instance IDs per call
        ...
        # This might happen if permissions are missing (ssm:DescribeInstanceInformation)
        print(f"Could not get SSM statuses in region {region}. Check IAM permissions. Error: {e}")
        return {} # Return an empty map so the script can continue
...
def get_all_ec2_instances(profile_name: str, region_input: str) -> Optional[List[Dict[str, Any]]]:
    ...
    for region in regions_to_scan:
        ...
        try:
            ...
            print(f"Found {len(regional_instances)} instances in {region}. Checking SSM status...")
